...
inhibits_reuse(#b_set{op=phi,args=Args}, New) ->
    foldl(fun({Value,_}, Bool) ->
                  sets:is_element(Value, New) andalso Bool
          end, true, Args);
inhibits_reuse(#b_set{op=put_map,args=Args}, New) ->
    [_,Map|Updates] = Args,
    sets:is_element(Map, New) orelse cannot_reuse(Updates, New);
...
cannot_reuse([_,Value|Updates], New) ->
