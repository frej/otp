            ...
            case {Op,Args,Element} of
                ...
                {get_tuple_element,[#b_var{}=Tuple,#b_literal{val=Idx}],_} ->
                    ...
                    IsComp =
                        case TupleType of
                            #t_tuple{elements=Es} ->
                                Type = maps:get(Idx + 1, Es, any),
                                ?DP(" type: ~p~n", [Type]),
                                fiv_elem_is_compatible(Element, Type);
                            #t_union{tuple_set=TS} ->
                                fiv_element_is_compatible_with_tuple_set(
                                  Element, Idx, [T || {_,T} <- TS])
                             end,
                    ?DP("~p is ~scompatible with ~p~n",
                        [Element,
                         case IsComp of true -> ""; false -> "not " end,
...
fiv_element_is_compatible_with_tuple_set(Element, TupleIdx,
                                         [#t_tuple{exact=true,size=S}|TS])
  when TupleIdx >= S ->
    %% It can't be this tuple as the index is out of bounds.
    fiv_element_is_compatible_with_tuple_set(Element, TupleIdx, TS);
fiv_element_is_compatible_with_tuple_set(Element, TupleIdx,
                                         [#t_tuple{elements=Es}|TS]) ->
    Type = maps:get(TupleIdx + 1, Es, any),
    fiv_elem_is_compatible(Element, Type) orelse
        fiv_element_is_compatible_with_tuple_set(Element, TupleIdx, TS);
fiv_element_is_compatible_with_tuple_set(_Element, _TupleIdx, []) ->
    false.
